...
7. **Emit.** `npm run herald:emit -- --note "<what this run was>"`. The batch
   lands whole or not at all, and the Trace is written before you report.
...
- **Notion is read-only.** The index comes from a local export. Connector
  read tools may look at one page; write tools are never called.
...
| `npm run herald:emit -- [--note "…"] [--dry]` | Land the batch, log the run |
