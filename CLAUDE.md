...
| The AI layer: models, tiers, fallbacks, paid gate, budget | `packages/ai/src/models.js` (the one file to change models) · `router.js` · `npm run models:verify` |
| The runnable AI agents, their tools and the runner | `packages/agents/src/` — `registry.js`, `agents/*.js`, `tools.js`, `runner.js` · `docs/AI.md` · `npm run agent -- <id>` |
| The persona library (agency-agents, MIT) | `packages/agents/src/personas.generated.js` — generated by `npm run personas:import -- <clone>`; never edited |
