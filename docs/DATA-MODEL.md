...
### The AI layer (0013, `docs/AI.md`)
**agents** — `id` (pk: an id from `packages/agents/src/registry.js`, or `crafted-<slug>`) · `enabled` (off until switched on) · `schedule` (5-field cron, UTC; empty is manual) · `config` (jsonb: the code definition as last run, or a crafted agent's whole definition) · `custom` (crafted in THE AGENT GARAGE) · `lock_run_id` · `lock_until` (taken with one conditional update — `lock_until is null or past` — so two runs of one agent never overlap) · `last_run_at` · `last_status`.
**outputs** — `id` (`OUT-YYYYMMDD-NNN`) · `run_id` · `agent` · `type` (summary | draft | verdict | trend | note) · `room` · `title` · `content` · `data` (jsonb) · `source_ref` (a Notion page id) · `source_hash` (its last edit, to skip it when unchanged) · `status` (pending | approved | rejected — only the operator moves it) · `model` · `reviewed_at`.
**model_usage** — one row per model call, success or failure: `at` · `run_id` · `agent` · `purpose` · `provider` · `model` (the key in `models.js`) · `model_id` · `served_model` · `free` · `ok` · `error` · `tokens_in` · `tokens_out` · `cost_usd` · `cost_gbp` · `latency_ms`. The router reads this month's paid rows before every paid call.
`agent_runs` gains `cost_gbp`, `steps`, `duration_ms`, `trigger` (manual | cron) and `output_ref`. AI runs are recorded under their owner's roster name (`ORACLE`, `SCRIBE`, `ARCANE`, `HERALD`) with `skill` = the agent id, so the floor's status lamps count them.
