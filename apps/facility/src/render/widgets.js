...
const warroom = (store, brain) => { const rows = brain?.brief?.blocks?.rooms || []; return `<h3>Where the work is</h3>${rows.length ? table(Object.keys(rows[0]).map((h) => h.replace(/_/g, ' ')), rows.map((r) => `<tr>${Object.values(r).map((v) => `<td>${esc(v)}</td>`).join('')}</tr>`)) : '<p class="empty">The brief has no ROOMS block.</p>'}<p class="ash">VECTOR ranks the ventures by what is compounding and names what to stop doing. Top three moves land here once Counsel is wired.</p>`; };
const inventor = (store, brain) => `<p>${(brain?.files?.['00-Inbox'] || []).length - 1} item${(brain?.files?.['00-Inbox'] || []).length === 2 ? '' : 's'} in the inbox.</p><p class="ash">SPARK takes an idea through market, competition, economics, MVP, cost and risk, then returns BUILD, WATCH or KILL.</p>`;
const intel = () => '<p class="empty">Web research is not wired. CIPHER produces the daily intelligence — opportunity, threat, signal, action — once it is.</p>';
const dealroom = () => '<p class="empty">The CRM is not wired. ENVOY researches a company, drafts the outreach and prepares the meeting brief. Sends nothing without your word.</p>';
const lounge = (store) => { const mins = Math.round((Date.now() - (store.sessionStart || Date.now())) / 60000); return `<p>You have been in the building <b>${mins} min</b> this session.</p><p class="ash">EMBER is the only agent whose job is to tell you to leave.</p>`; };
