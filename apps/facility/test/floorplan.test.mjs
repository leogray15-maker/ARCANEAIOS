 ...
 */
import { PLAN, PW, PH, buildGraph, path, VCORR, HALL_Y, HALL_H } from '../src/config/floorplan.js';
...
for (const p of PLAN) for (const c of VCORR) {
  const [x, , w] = p.rect;
  ok(c - 18 >= x + w || c + 18 <= x, `${p.id} overlaps corridor at ${c}`);
}
...
console.log(`✓ floorplan: 20 rooms, ${g.nodes.size} nodes, ${pairs} reachable pairs, longest path ${longest} nodes, ${PW}x${PH}`);
