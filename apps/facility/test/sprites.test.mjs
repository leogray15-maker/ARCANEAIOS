...
import { BODIES, validateBody, validateKits, composeFrame, paletteFor } from '../src/render/sprites.js';

const problems = [
  ...validateBody('crew', BODIES.crew.body, 12, 18),
  ...validateBody('arcane', BODIES.arcane.body, 14, 22),
  ...validateKits(),
];
let frames = 0;
for (const a of AGENTS) {
  const pal = paletteFor(a);
  for (const facing of ['front', 'back', 'side']) for (const cel of ['stand', 'stepA', 'stepB']) {
...
console.log(`✓ sprites: ${AGENTS.length} agents × 9 frames = ${frames} matrices valid (crew 12×18, ARCANE 14×22)`);
